# ====================================================================
# PIJAK DATABASE SEEDER & FULL VERIFIER
# - Membersihkan database
# - Compile backend
# - Menjalankan semua command yang relevan
# - Mencetak raw output + JSON bila ada
# - Menandai PASS / FAIL / ANOMALY
# ====================================================================
...
Write-Host ""
Write-Host "[1] Membersihkan database lama..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[2] Compile ulang C++..." -ForegroundColor Yellow
...
function Get-JsonLineFromOutput {
    param([string]$Text)
...
function Invoke-PijakCase {
    ...
    $exit = $LASTEXITCODE
    $jsonLine = Get-JsonLineFromOutput -Text $stdout

    $hasOkTrue = $false
    $hasOkFalse = $false

    if ($jsonLine) {
        if ($jsonLine -match '"ok"\s*:\s*true') { $hasOkTrue = $true }
        if ($jsonLine -match '"ok"\s*:\s*false') { $hasOkFalse = $true }
    }

    $status = "ANOMALY"

    if ($exit -eq 0 -and $hasOkTrue) {
        $status = "PASS"
    }
    elseif ($hasOkFalse) {
        if ($Expect -eq "FAIL") {
    ...
    elseif (-not $JsonRequired -and $exit -eq 0) {
        $status = "PASS_TEXT"
    }

    if ($Expect -eq "PASS" -and $status -notin @("PASS", "PASS_TEXT")) {
        $status = "ANOMALY"
    }

    $compact = ($stdout -replace "`r", "" -replace "`n", " | ").Trim()
    ...
    $entry = [PSCustomObject]@{
        ...
        Exit      = $exit
        Status    = $status
        Json      = $(if ($jsonLine) { $jsonLine } else { "" })
        Output    = $compact
        Command   = ($Args -join " ")
    }
    ...
    if (Test-Path $CsvFile) {
        ...
        $entry | Export-Csv -Path $CsvFile -NoTypeInformation
    }
    ...
    switch ($status) {
        ...
        "PASS_TEXT" { Write-Host "[PASS/TEXT] $Name" -ForegroundColor Green }
        "EXPECTED_FAIL" { Write-Host "[FAIL] $Name (sesuai ekspektasi)" -ForegroundColor DarkYellow }
        default { Write-Host "[ANOMALY] $Name" -ForegroundColor Red }
    }

    Write-Host "CMD : $Exe $($Args -join ' ')" -ForegroundColor DarkGray
    if ($jsonLine) {
        Write-Host "JSON: $jsonLine" -ForegroundColor Yellow
    } else {
        Write-Host "JSON: <tidak ada JSON terdeteksi>" -ForegroundColor DarkYellow
    }
...
Write-Host ""
Write-Host "[3] Seeding Users..." -ForegroundColor Yellow
...
$results += Invoke-PijakCase -Name "login akun tidak ditemukan" -Args @(
    "login","--auth","tidakada@pijak.com","--password","123"
) -Expect "FAIL"
...
Write-Host ""
Write-Host "[4] Seeding Opportunities..." -ForegroundColor Yellow
...
$results += Invoke-PijakCase -Name "delete-opportunity temp O99" -Args @(
    "add-opportunity","--id","O99","--creator_id","U01","--title","Temp Delete","--type","Freelance","--reward","Rp1"
) -Expect "PASS"
$results += Invoke-PijakCase -Name "delete-opportunity O99" -Args @(
    "delete-opportunity","--id","O99"
) -Expect "PASS"
...
Write-Host ""
Write-Host "[5] Seeding Proposals..." -ForegroundColor Yellow
...
$results += Invoke-PijakCase -Name "get-proposal P01" -Args @(
    "get-proposal","--id","P01"
) -Expect "PASS"
...
Write-Host ""
Write-Host "[6] Seeding Workrooms..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[7] Seeding Tasks..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[8] Stack / Activities..." -ForegroundColor Yellow
...
$results += Invoke-PijakCase -Name "show-activities" -Args @("show-activities") -Expect "ANY" -JsonRequired:$false

Write-Host ""
Write-Host "[9] Sorting / Hash / Tree / Graph..." -ForegroundColor Yellow
...
$results += Invoke-PijakCase -Name "show-opportunity-tree" -Args @("show-opportunity-tree") -Expect "ANY" -JsonRequired:$false
...
$results += Invoke-PijakCase -Name "show-graph" -Args @("show-graph") -Expect "ANY" -JsonRequired:$false
